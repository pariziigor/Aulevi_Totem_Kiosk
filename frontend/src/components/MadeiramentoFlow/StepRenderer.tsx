  ...
  const renderStep = () => {
    switch (currentStep) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 xl:gap-12 w-full max-w-4xl">
              {[
                ...
                >
                  ...
                  <div>
                    <span className="text-xl md:text-2xl xl:text-4xl font-bold text-slate-700 block">{option.label}</span>
                    <span className="text-sm md:text-base xl:text-lg text-slate-400 mt-1 md:mt-2 block">{option.sub}</span>
                  </div>
                </motion.button>
              ))}
            </div>
      ...
      case MADEIRAMENTO_FLOW_STEPS.DIMENSOES: {
        ...
        return (
          <motion.div key="s1" variants={stepVariants} initial="initial" animate="animate" exit="exit" className="flex flex-col lg:flex-row items-center justify-center gap-6 md:gap-8 xl:gap-14 w-full px-4 md:px-0">
          ...
          <motion.div key="s2" variants={stepVariants} initial="initial" animate="animate" exit="exit" className="flex flex-col items-center w-full px-4 md:px-0">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 xl:gap-6 w-full max-w-4xl mb-6">
              {TIPOS_TELHA.map((telha) => (
                ...
                >
                  <img src={TELHA_ICONS[telha]} alt={telha} className="w-12 h-12 md:w-20 md:h-20 xl:w-28 xl:h-28 object-contain" />
                  <span className={`text-sm md:text-lg xl:text-xl font-bold text-center leading-tight ${tipoTelha === telha ? "text-orange-700" : "text-slate-700"}`}>
                    {telha}
                  </span>
                </motion.button>
              ))}
