...
export const CitySearchModal: React.FC<CitySearchModalProps> = ({
  ...
  const handleKey = (key: string) => {
    ...
    onSearchChange((searchValue + key).toLowerCase());
  };
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 30 }}
      className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-3 md:p-4 lg:p-6"
    >
      <div className="bg-slate-50 w-full max-w-4xl rounded-2xl md:rounded-[2rem] shadow-2xl flex flex-col overflow-hidden h-[90vh] lg:h-auto lg:max-h-[900px]">
        <header className="bg-white border-b border-slate-200 px-4 md:px-6 py-3 md:py-4 flex justify-between items-center flex-none">
          ...
          <button
            type="button"
            onClick={onClose}
            className="bg-slate-100 text-slate-600 p-2 md:p-3 rounded-full hover:bg-slate-200"
          >
        ...
        <div className="p-4 md:p-6 bg-white border-b border-slate-100 flex-none">
          <div className="w-full bg-slate-50 border-2 border-orange-500 ring-2 md:ring-4 ring-orange-50 rounded-xl md:rounded-2xl h-14 md:h-16 lg:h-16 flex items-center px-4 md:px-6 shadow-inner">
            <input
              type="text"
              value={searchValue}
        ...
        <div className="hidden lg:flex flex-none bg-white p-6 flex-col justify-center gap-2 xl:gap-3">
          {CITY_KEYBOARD_LAYOUT.map((row: string[], rowIndex: number) => (
